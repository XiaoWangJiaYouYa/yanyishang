...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    remaining = [item for item in paragraphs if select(item)]
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    total_len=len(typed_words)
    total_reference = len(reference_words)
    if total_len==0 and total_reference==0:
        return 100.0
    if total_len==0 or total_reference==0:
        return 0.0
    if total_reference<total_len:
        correct_nums=0
        for i in range(total_reference):
            if typed_words[i]==reference_words[i]:
                correct_nums+=1
                pass
            pass
        return round((float(correct_nums)/float(total_len))*100,2)
    else:
        correct_nums = 0
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    length_words = float(float(len(typed))/5.0)
    return round(length_words*float(60.0/float(elapsed)),2)
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        for item in valid_words:
            mid_value=diff_function(typed_word,item,limit)
            if mid_value<min_diff or (result==typed_word and mid_value==min_diff):
                result=item
                min_diff=mid_value
                pass
            pass
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def feline_flips_recursion(start,goal,current_change,limit):
        if start=="" and goal=="":
            return 0
        else:
            if start=="":
            ...
            if goal=="":
                ...
                if start[0]==goal[0]:
                    return feline_flips_recursion(start[1:],goal[1:],current_change,limit)
                else:
                    current_change+=1
                    if current_change>limit:
                        return 1
                    else:
                        return 1+feline_flips_recursion(start[1:],goal[1:],current_change,limit)
        pass
    if abs(len(start)-len(goal)>limit):
        return limit+1
    else:
        return feline_flips_recursion(start,goal,0,limit)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(sofar)):
        if sofar[i]==prompt[i]:
            ...
            pass
        pass
    progress=float(same_char)/float(len(prompt))
...
def time_per_word(words, times_per_player):
    ...
    for time_list in times_per_player:
        temp_list=[]
        for times_index in range(1,len(time_list)):
            temp_list.append(time_list[times_index]-time_list[times_index-1])
            pass
        results.append(temp_list)
        pass
    return match(words,results)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            if time(match, player_index, word_index) < min_time:
                min_time = time(match, player_index, word_index)
                player = player_index
        fastest[player].append(word_at(match, word_index))
    return fastest
    # END PROBLEM 10
